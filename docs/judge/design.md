...
6. **채점 엔진은 인터페이스 뒤에** - `JudgeEngine { run(language, source, inputs, limits) → RunResult[] }` 구현 `Judge0Engine`(prod) / `FakeJudgeEngine`(local·test - 미리 정한 출력 반환). 인증 `stub/oidc` 모드와 같은 패턴(`ondal.judge.engine: judge0|fake`). 이유: Judge0 없이 BE·FE 개발과 테스트가 돌아가야 하고, **Judge0 1.13.1 의 cgroup v1 의존(infra.md 3절)이 서버에서 풀리지 않으면 엔진만 갈아 끼울 수 있어야 한다**
