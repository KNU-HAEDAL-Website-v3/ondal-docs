...
1. ~~**문제 = 과제 + 테스트케이스, 별도 problems 테이블 없음**~~ **(2026-09-15 결정 17 로 폐기 - 아래 참고)** - CLAUDE.md 원칙 1("OJ 문제 = 분반 없는 과제")의 P2 단계. `test_cases(assignment_id FK, position, input, expected_output, is_public)` + `assignments.time_limit_ms`·`memory_limit_mb`(NULL = 기본값). 테스트케이스 0개 = 지금처럼 제출만 받는 과제 → 기존 과제·제출 동작은 하나도 바뀌지 않는다
...
7. **언어 6종은 FE 셀렉트 그대로, 매핑은 설정** - `C, C++, Java, Python 3, JavaScript, TypeScript`(SubmissionForm `LANGUAGES`) → Judge0 `language_id` 는 `application.yml` `ondal.judge.languages` 표(코드 아님). 자동 채점 문제에서 매핑 없는 언어로 제출하면 400. 과제별 허용 언어 제한은 두지 않음(P2 단순화 - C언어 반에서 파이썬으로 내면 운영진이 코멘트로 처리). **2026-09-19 보완([결정 11](../decisions/11-%EB%AC%B8%EC%A0%9C-%EC%9D%80%ED%96%89-%EB%82%9C%EC%9D%B4%EB%8F%84-%ED%97%88%EC%9A%A9-%EC%96%B8%EC%96%B4-%EB%B2%88%EB%93%A4-%EA%B0%80%EC%A0%B8%EC%98%A4%EA%B8%B0.md)): 문제별 허용 언어(`problems.allowed_languages`, V9)는 둔다** - 언어 특화 문제에만 걸고, 걸린 문제는 과제 제출·연습 제출 모두 그 언어만(400)
...
## 결정 17 - 문제(Problem)를 과제에서 떼어낸다 (2026-09-15, V7)

### 무엇이 문제였나

- 결정 1("문제 = 과제")에서는 같은 문제를 다시 내려면 **과제를 새로 만들고 테스트케이스까지 복제**해야 했음
- 복제본끼리 채점 기준이 갈라짐: 한쪽 케이스의 오류를 고쳐도 다른 쪽은 그대로 → 같은 문제인데 판정이 달라짐
- 가정이 아니라 **구조상 확정적인 결함**. 전역 유일 `problem_no` 가 분반 종속 테이블에 있던 것도 두 개념이 뭉쳐 있다는 신호였음

### 어떻게 바꿨나

| 개념 | 가진 것 |
|---|---|
| **Problem** (분반 무관) | 번호(전역 유일) · 제목 · 본문 · 실행 제한 · **테스트케이스** · 태그 |
| **Assignment** (배정) | 분반 · **문제** · 차시 · 마감 |

- **재출제 = 배정 한 줄 추가.** 테스트케이스는 복제하지 않고 문제 하나를 공유 → 기준이 갈라지지 않음
- 검토했다 버린 대안: `assignments.origin_assignment_id`(자기 참조). 사실상 problems 의 열화판 - 원본 과제가 여전히 `cohort_id`·`due_at` 을 들고 라이브러리 노릇을 하고, 원본을 지우면 계보가 끊김

### 채점 스코프

- 설정·테스트케이스·출제 도구·공개 예시 → **문제 스코프** `/api/problems/{problemId}/judge/*`
- 재채점만 **과제 스코프** 유지 - "내 반 제출만 다시 돌린다"는 분반 운영 동작이라서
- 케이스를 고치면 **그 문제로 채점된 제출 전부**(여러 분반의 과제 제출 + HOJ 연습)가 재채점 대상 - 기준이 바뀌면 그 기준으로 매긴 판정은 전부 낡은 값이기 때문
- 채점 설정은 **분반 보관과 무관** - 문제의 것이므로

### HOJ 연습 제출

- 분반과 무관하게 문제를 풀어 채점받는 경로. 로그인한 누구나(분반 미소속 부원 포함)
- **과제 제출과 같은 파이프라인** (BE CLAUDE.md 원칙 1): 같은 `submissions` 테이블, 같은 `enqueueIfJudged` → `JudgeWorker` → 서버 비교기
- 다른 점은 대상이 `assignment` 가 아니라 `problem` 이라는 것뿐 → 마감·지각·운영진 코멘트·현황판이 전부 없음
- DB `CHECK num_nonnulls(assignment_id, problem_id) = 1` 로 "둘 중 정확히 하나"를 강제

### 권한

| 대상 | 조회 | 쓰기 |
|---|---|---|
| 문제 | 로그인 누구나 | **운영진 이상**(`@OperatorAnywhere` - 분반이 없어 `{cohortId}` 로 판정 불가) |
| 태그 | 로그인 누구나 | **ADMIN 만** - 자유 생성이면 "DP / 다이나믹프로그래밍 / dp" 로 갈라져 분류가 쓸모없어짐 |
| 연습 제출 | 본인 것만 | 로그인 누구나 |

### 마이그레이션 (V7, 파괴적)

- 기존 과제 1건당 문제 1건 이관(번호·제목·본문·제한) → `assignments` 에서 그 열들 제거
- 적용 전 **DB 덤프 백업 필수**
- 리허설로 확인한 것: 이관 1:1 · 백필 정확성(`assignments`/`test_cases`/`judge_results`) · 제출·코멘트 보존 · 새 CHECK 4종 동작 · **이관된 DB 로 앱 기동(ddl-auto=validate) 성공**
