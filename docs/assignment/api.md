...
| 15 | 과제 등록 | `POST /api/cohorts/{cohortId}/assignments` | 운영진 이상 | 201 + Location | 400(입력 검증), 403, 404(분반 없음), 409(보관 분반) |
| 16 | 과제 수정 | `PUT /api/cohorts/{cohortId}/assignments/{assignmentId}` | 운영진 이상 | 200 (전체 교체) | 400, 403, 404, 409(보관 분반) |
...
- 응답은 다섯 엔드포인트 모두 `AssignmentResponse {id, sessionNo, title, description, dueAt, createdAt}` 하나
- 등록·수정 요청은 `AssignmentCreateRequest` / `AssignmentUpdateRequest` - 필드·검증 동일: `sessionNo`(선택, 1 이상 정수 - design.md 결정 6), `title`(필수, 최대 200자), `description`(선택, 최대 10000자), `dueAt`(필수, 과거 시각 허용 - design.md 결정 3)
