...
- ~~P2 이후로 이연: 자동 채점, 알림, 멘토 코멘트~~ → 2026-09-16 기준 갱신
  - 자동 채점: **P2 도입·운영 가동** (judge 슬라이스, Judge0 VM 연결. judge/design.md)
  - 멘토 코멘트: **P2 도입 완료** - 제출 1건에 코멘트 1개, **점수는 두지 않음** (결정 14·18)
  - 알림: **하지 않기로 확정**(2026-09-14) - 과제가 필수가 아니라 독촉 자동화가 불필요
- ~~P3: OJ 모드·대회~~ → OJ 는 **P2 로 당겨져 HOJ 로**(2026-09-15 별도 앱 분리 → 2026-09-19 **같은 앱 안의 모드**로 정정, 결정 9), 대회만 P3 이후
  - 문제는 분반과 무관한 라이브러리(`problems`, V7) - "OJ 문제 = 분반 없는 과제" 원칙은 폐기
  - HOJ(문제 은행)와 Ondal(과제 제출)은 메뉴(셸)만 나눴고 **앱도 백엔드도 하나** - 채점·사용자·세션을 두 벌로 만들지 않음. 오갈 때 "~로 이동할까요?" 확인 팝업
...
| 3 ⚖ | 파일 제한 | **확정(2026-08-26): zip 단일 확장자 + 20MB** → **개정(2026-08-26 PM 리뷰): 10MB** - 온프렘 디스크 보호. 심화반 과제가 다른 형식을 요구하면 화이트리스트 확장 ([submission/design.md](submission/design.md) 결정 3·14) |
...
| 10 ⚖ | 회원가입 방식 | **해소(2026-09-19, [결정 10](decisions/10-%EC%8A%B9%EC%9D%B8-%EA%B2%8C%EC%9D%B4%ED%8A%B8-%EC%B2%AB-%EB%A1%9C%EA%B7%B8%EC%9D%B8%EC%9D%80-%EC%8A%B9%EC%9D%B8-%EB%8C%80%EA%B8%B0.md))**: 홈페이지 **구글 로그인**으로 계정은 스스로 생기고, Ondal 에서는 **승인 대기(PENDING)** 로 시작 - 운영진 이상이 승인하거나 분반에 배정해야 화면이 열림. SMTP 불필요(Ondal 은 메일을 보내지 않고, 구글 계정은 비밀번호 찾기가 없음). 아래 3절-10 상세는 결정 전 검토 기록 |
...
### 3절-10 상세 - 회원가입 방식 (2026-09-16 회의 안건) - 2026-09-19 해소, 아래는 기록

> 결론([결정 10](decisions/10-%EC%8A%B9%EC%9D%B8-%EA%B2%8C%EC%9D%B4%ED%8A%B8-%EC%B2%AB-%EB%A1%9C%EA%B7%B8%EC%9D%B8%EC%9D%80-%EC%8A%B9%EC%9D%B8-%EB%8C%80%EA%B8%B0.md)): Keycloak 에 구글 IdP 를 붙여 자율 가입(선택지 B/C 의 인증을 구글이 대신), Ondal 쪽 승인 게이트로 외부인 유입을 막는다. 관리자 콘솔 수동 생성(A)은 더 이상 필요 없다

**지금 상태**

- Ondal 은 비밀번호를 직접 받지 않음 - 로그인은 홈페이지 Keycloak OIDC 뿐 ([결정 7](decisions/7-%ED%99%88%ED%8E%98%EC%9D%B4%EC%A7%80-sso-keycloak-oidc-be-%EC%BD%94%EB%93%9C-%EA%B5%90%ED%99%98.md))
- 계정 생성 경로가 **관리자 콘솔 수동 생성 하나뿐** - 부원이 스스로 가입할 방법이 없음
- Keycloak 에 self-registration 기능은 내장돼 있음 (Realm Settings → Login → User registration) - 직접 구현할 것은 없음

**선택지**

| 안 | 내용 | 장점 | 비용·위험 |
|---|---|---|---|
| A. 현행 유지 | 관리자가 콘솔에서 생성 | 추가 작업 0, 아무나 못 들어옴 | 인원 늘면 관리자 병목. 테스트 주간에 계정 요청이 몰림 |
| B. 자율 가입 + 이메일 인증 | 토글 ON + 도메인 제한(경북대) + 학번·실명 필수 | 병목 해소, 명부와 대조 가능 | **SMTP 발신 계정 필요**. 도메인 밖 부원 예외 처리 필요 |
| C. 자율 가입(인증 없음) | 토글만 ON | 즉시 적용 | 외부인 유입·중복 계정. 권장하지 않음 |

**어느 안이든 먼저 해야 하는 것**

- **Edit username OFF** - `username` 이 Ondal `login_id` 와 계정을 잇는 키라, 사용자가 바꾸면 계정이 끊어짐 (결정 7 전제)
- 가입 후에도 **분반 배정은 운영진이 함** - 가입 = 로그인 가능, 수업 접근은 별개 (permissions.md 2층 권한)

**B 를 고를 경우 남는 것**

- realm SMTP 발신 계정 확보 → Keycloak Email 설정
- 홈페이지 가입 페이지를 쓸지, Keycloak 가입 화면을 쓸지 결정 → 전자면 URL 확정 후 FE 로그인 화면에 링크 연결
- 결정 후 `docs/decisions/` 에 기록 추가
