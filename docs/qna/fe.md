...
### 1-1. 답변 (2026-09-14 편입, design.md 결정 11)

- 질문 상세 하단 "답변" 영역: 목록(오래된 순, 작성자 배지·KST·본문 줄바꿈 보존) + 작성 폼(분반 소속 누구나, content 10000, 임시 저장) + 답변별 인라인 수정(`canEdit`)·삭제(`canDelete`, 남의 답변이면 작성자 명시 확인 문구)
- 질문 목록 행에 "답변 N" 배지(`answerCount`, 0이면 회색). 질문 삭제 확인 문구에 "답변 N건도 함께 삭제"
- 보관 분반: 답변 열람만, 폼 대신 안내 문구
- API 클라이언트 `src/api/answers.ts` - 답변 쓰기 성공 시 질문 캐시 접두사 무효화(목록·상세 answerCount·답변 목록 한 번에)
