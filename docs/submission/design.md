...
- 제외: 자동 채점(P2 - Judge0, 2026-09-14 필수 확정·착수 예정), ~~멘토 코멘트·점수(P2 - 열만 존재, 항상 NULL)~~ → **2026-09-14 결정 18: 운영진 코멘트 도입, 점수 폐기 (5절)**, 분반 영구 삭제(별도 슬라이스 - guide 8절 결정 5), 분반 전체 제출 기록 페이지(결정 8)
...
- [x] P2 제출 코멘트 BE - 결정 18, ondal-BE 이슈 #37 → PR #38 머지 (2026-09-14, SubmissionCommentApiTest 2건 포함 전체 195건 통과, Flyway V5)
- [ ] P2 제출 코멘트 FE - ondal-FE 이슈 #37 → PR #38 (헤드리스 19건 통과, 서버 BE 재빌드 후 머지)
...
## 5. 제출 코멘트 (2026-09-14 P2 - PM 확정)

- 배경: P2 논의(2026-09-14)에서 PM 결정 - "제출 건에 대한 코멘트는 좋은데 점수는 필요 없음. 채점 결과는 채점 엔진(Judge0)이 말한다"
- 결정 18. **제출 코멘트 = 제출의 속성 3열, 제출 1건에 1개, 점수 없음**
  - 저장: `submissions.mentor_comment`(V1 예약 열 그대로) + `commented_by`(FK users, 마지막으로 남긴 운영진) + `commented_at`. 세 열은 함께 NULL 이거나 함께 값. `score` 열은 제거 (Flyway `V5__submission_comment.sql`, schema.md 결정 14)
  - 별도 엔티티·읽기 API 없음 - 학생은 기존 #19(`hasComment`)·#20(`comment`)으로 읽는다. 코멘트 이력·스레드는 두지 않음(질문·토론은 Q&A 게시판 몫)
  - 쓰기: #45 `PUT .../submissions/{submissionId}/comment`(남기기·덮어쓰기, 응답 = 갱신된 제출 상세) / #46 `DELETE`(지우기, 멱등 204). 권한은 `@CohortRole(OPERATOR)` 한 줄 - 운영진 이상, 작성자 무관(운영진 팀이 함께 검토). 보관 분반 409, 스코프 불일치 404, 빈 내용 400
  - 재제출과의 관계: 코멘트는 제출 행에 붙으므로 재제출(새 행)에는 코멘트가 없다 → 현황판 `latestCommented`(최신 제출에 코멘트가 있는가)가 "아직 검토하지 않은 최신 제출"을 드러낸다. 과거 제출의 코멘트는 이력에 남는다
  - FE: 제출 단건 펼침 뷰 하단 "운영진 코멘트" 상자 - 운영진(`canManage`)은 열람하던 자리에서 바로 작성(코멘트 없으면 폼이 바로 열림)·인라인 수정·지우기(confirm). 학생은 코멘트가 있을 때만 상자를 보고, 내 기록 행에 "코멘트" 배지, 현황판에는 "코멘트" 열(남김/아직) - [fe.md](fe.md) 1절
  - 알림 없음: 디스코드·이메일 알림은 P2 제외 확정(2026-09-14) - 학생은 과제 상세에 들어와서 배지로 확인
